...
<body>
    ...
    <!-- Phone Login Overlay (Default visible for every new visit/session) -->
    <div id="loginOverlay">
        <div class="login-box">
            <h3>📱 Phone Login</h3>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // Fix 1: Har naye session par strictly unlogged/logout karke login screen dikhane ke liye sessionStorage use kiya hai
        ...
        // Fix 2: Subscription buttons click listener properly bind kar diya gaya hai
    ...
    function checkPhoneLoginState() {
        ...
        let activePhone = sessionStorage.getItem('cricket_active_phone');
    ...
    function handlePhoneLogin() {
        ...
        sessionStorage.setItem('cricket_active_phone', phoneInput);
        document.getElementById('loginOverlay').classList.add('hidden');
        initUserSession(phoneInput);
    }

    function handleLogout() {
        sessionStorage.removeItem('cricket_active_phone');
        location.reload();
    }
    ...
    function initUserSession(phoneNum) {
        ...
        userRef.once('value', (snapshot) => {
            // Fix 3: 9569981484 number ke liye 5000 points aur baki sabhi naye users ke liye 50 points set kiye gaye hain
    ...
    function buySubscription(cost, planName, extraDays, extraMins) {
        let myPhone = sessionStorage.getItem('cricket_active_phone');
        if(!myPhone) {
